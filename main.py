...
if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    #kaç yaşındasın bot
    print("kyb")
    bot_adi = "@XAOS_Namebot"
    bot_token = "5805574931:AAEK1HWFX0LdfdLCioJZxf8rhDu04wA5HKA"
...
kurucu_id = 5663585448

admins = [kurucu_id, 5663585448]
...
async def log_gonder(**kwargs):
    ...
    try:
        await bot.send_message(-1001826411396, f"""
<b> ~~ 📢 Log Kaydı ~~</b>
...
Eylem: <code>{kwargs.get('eylem','')}</code>
    ...
    """, disable_web_page_preview=True)
        ...
        #    bot.send_message(kurucu_id, str(e))
    try:
        await bot.set_chat_title(-1001826411396, f"Bot Log - {len(oyunlar)}")
    except Exception as e:
        if "chat not found" in str(e):
            pass
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001826411396, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
